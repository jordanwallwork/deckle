<script lang="ts">
  ...
  async function handleLink() {
    ...
    isLinking = true;
    try {
      await adminApi.updateSampleDataSource(linkTarget.id, selectedDataSourceId);
      await invalidateAll();
      closeLinkModal();
    } catch (err) {
  ...
  async function handleUnlink() {
    ...
    isLinking = true;
    try {
      await adminApi.updateSampleDataSource(linkTarget.id, null);
      await invalidateAll();
      closeLinkModal();
    } catch (err) {
...
<Dialog bind:show={showModal} title="New Sample Component" maxWidth="600px" onclose={closeModal}>
  {#if !selectedType}
    <p class="type-hint">Select a component type to create a sample component.</p>
    <div class="type-selection">
      <button class="type-card" onclick={() => selectType('card')}>
        <div class="type-icon">🃏</div>
        <h4>Card</h4>
        <p>Create a sample card</p>
      </button>
      <button class="type-card" onclick={() => selectType('playermat')}>
        <div class="type-icon">📋</div>
        <h4>Player Mat</h4>
        <p>Create a sample player mat</p>
      </button>
    </div>
  {:else}
    <Button variant="text" onclick={() => (selectedType = null)}>← Back to component types</Button>
...
<Dialog show={!!linkTarget} title="Link Data Source" maxWidth="600px" onclose={closeLinkModal}>
  {#snippet children()}
  ...
  {#snippet actions()}
    ...
    {#if data.sampleDataSources.length > 0}
      <Button variant="primary" onclick={handleLink} disabled={isLinking || !selectedDataSourceId}>
