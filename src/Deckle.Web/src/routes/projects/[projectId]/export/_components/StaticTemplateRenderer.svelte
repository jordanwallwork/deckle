  ...
  function borderToCss(border: any | undefined): string[] {
    ...
    if (hasIndividualSides) {
      ...
      for (const side of sides) {
        ...
        if (sideProps) {
          if (sideProps.width !== undefined) {
            styles.push(
              `border-${side}-width: ${dimensionValue(sideProps.width)}`
            );
          }
          if (sideProps.style) {
      ...
      if (border.width !== undefined)
        styles.push(`border-width: ${dimensionValue(border.width)}`);
  ...
  function buildStyle(element: TemplateElement): string {
    ...
    if (element.position === "absolute") {
      ...
      if (element.x !== undefined)
        styles.push(`left: ${dimensionValue(element.x)}`);
      if (element.y !== undefined)
        styles.push(`top: ${dimensionValue(element.y)}`);
    } else {
    ...
    if (element.type === "container") {
      ...
      if (container.display === "flex" && container.flexConfig) {
        ...
        if (fc.justifyContent)
          styles.push(`justify-content: ${fc.justifyContent}`);
        if (fc.alignItems) styles.push(`align-items: ${fc.alignItems}`);
        if (fc.alignContent)
          styles.push(`align-content: ${fc.alignContent}`);
        if (fc.gap !== undefined) styles.push(`gap: ${fc.gap}px`);
        if (fc.rowGap !== undefined) styles.push(`row-gap: ${fc.rowGap}px`);
        if (fc.columnGap !== undefined)
          styles.push(`column-gap: ${fc.columnGap}px`);
      }

      // Padding and margin
      if (container.padding)
        styles.push(`padding: ${spacingToCss(container.padding)}`);
      if (container.margin)
        styles.push(`margin: ${spacingToCss(container.margin)}`);
      ...
      if (container.background) {
        ...
        if (bg.image) {
          ...
          if (bg.image.repeat)
            styles.push(`background-repeat: ${bg.image.repeat}`);
          if (bg.image.position)
            styles.push(`background-position: ${bg.image.position}`);
        }
      }

      // Shadow
      if (container.shadow) {
        const shadows = Array.isArray(container.shadow)
          ? container.shadow
          : [container.shadow];
      ...
      if (container.dimensions) {
        ...
        if (d.width !== undefined)
          styles.push(`width: ${dimensionValue(d.width)}`);
        if (d.height !== undefined)
          styles.push(`height: ${dimensionValue(d.height)}`);
        if (d.minWidth !== undefined)
          styles.push(`min-width: ${dimensionValue(d.minWidth)}`);
        if (d.maxWidth !== undefined)
          styles.push(`max-width: ${dimensionValue(d.maxWidth)}`);
        if (d.minHeight !== undefined)
          styles.push(`min-height: ${dimensionValue(d.minHeight)}`);
        if (d.maxHeight !== undefined)
          styles.push(`max-height: ${dimensionValue(d.maxHeight)}`);
      }
    ...
    if (element.type === "text") {
      ...
      if (text.textDecoration)
        styles.push(`text-decoration: ${text.textDecoration}`);
      ...
      if (text.letterSpacing)
        styles.push(`letter-spacing: ${text.letterSpacing}px`);
      if (text.wordWrap) styles.push(`word-wrap: ${text.wordWrap}`);
      if (text.textTransform)
        styles.push(`text-transform: ${text.textTransform}`);
    ...
    if (element.type === "image") {
      ...
      if (image.objectPosition)
        styles.push(`object-position: ${image.objectPosition}`);
