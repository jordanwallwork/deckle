...
/**
 * Default dice size on the tabletop (matches StackZone card proportions so
 * they coexist visually). Dice don't have a design so they render as a
 * stylized square.
 */
export const DICE_SIZE_PX = 80;
...
function getTemplateDimensions(c: GameComponent): { widthPx: number; heightPx: number } {
  ...
  if (isDice(c)) {
    return { widthPx: DICE_SIZE_PX, heightPx: DICE_SIZE_PX };
  }
...
export function buildInitialTabletop(input: TabletopInitInput): TabletopInitResult {
  ...
  for (const c of components) {
    const { widthPx, heightPx } = getTemplateDimensions(c);
    templates[c.id] = {
      id: c.id,
      name: c.name,
      type: c.type,
  ...
  function placeInTableau(template: EntityTemplate): { x: number; y: number } {
    if (cursorX + template.widthPx > tableau.width - TABLEAU_PADDING) {
      cursorX = TABLEAU_PADDING;
      cursorY += rowHeight + TABLEAU_PADDING;
      rowHeight = 0;
    }
    const pos = { x: cursorX, y: cursorY };
    cursorX += template.widthPx + TABLEAU_PADDING;
    rowHeight = Math.max(rowHeight, template.heightPx);
    return pos;
  }
