<script lang="ts">
	...
	async function handleSave() {
		...
		try {
			const updated = await adminApi.updateSampleDataSource(data.dataSource.id, {
				name,
				jsonData
			});
	...
	async function handleDelete() {
		try {
			await adminApi.deleteSampleDataSource(data.dataSource.id);
...
<svelte:head>
	<title>{name} - Data Source Editor - Deckle</title>
</svelte:head>
